...
parser.add_argument(
    "--vel_path",
    type=str,
    default="/mnt/share_disk/KITTI/dataset/sequences/00/velodyne/",
    help="path to data",
)
parser.add_argument(
    "--bev_save_path",
    type=str,
    default="./KITTI_new_imgs/00/imgs/",
    help="path to data",
)
...
def getBEV(all_points):  # N*3
    """
        transform the input point cloud(single frame) into BEV image
    """
    all_points_pc = o3d.geometry.PointCloud()
    all_points_pc.points = o3d.utility.Vector3dVector(all_points)
    all_points_pc = all_points_pc.voxel_down_sample(voxel_size=0.4)
    # convert to np array
...
if __name__ == "__main__":
    ...
    for i in trange(len(bins_path)):
        ...
        # pcs = pcs.dot(rot_mat)
        # 裁剪出25m之内的点云，外面的会被删掉，这样可以使得BEV图像不至于太大，而且大小一致。
        ...
        img, _, _ = getBEV(pcs)
        print("BEV image size: ", img.shape)
